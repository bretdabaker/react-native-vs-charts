...
var Bar = React.createClass({
  ...
  render: function() {
    ...
    var antiValueFlex = 1 - this.props.ratio;
    // outter bar style
    var marginStyle = {
...
var BarSet = React.createClass({
  propTypes: {
    ...
    borderStyle: ViewStylePropTypes.borderStyle,
    borderWidth: ReactPropTypes.number,
    // Extract value from DatasetPropType
  ...
  _renderBars: function(value, index, dataValueSet) {
    ...
    var ratio = (value.value - this.props.minValue) / (this.props.maxValue - this.props.minValue);
    return (
      <Bar
        key={'bar-' + index}
...
var BarChart = React.createClass({
  ...
  _ensureValueScaleCoverRange: function(props) {
    ...
    var range = getRange(props.datasets);
    ...
    if (props.valueScale.unit) {
      invariant(
        (props.valueScale.unit < (range.max - range.min) && props.valueScale.unit > 0),
        'valueScale.unit: ' +
        props.valueScale.unit +
        ', must be within range and greater than zero: ' +
